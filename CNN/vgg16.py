...
result = inference_op(x, keep_prob= keep_prob, is_training = is_training)

cross_entrpy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=result, labels=tf.cast(y_, tf.int32))
learn_rate = tf.train.exponential_decay(learning_rate, training_step, num_train/batch_size, learning_decay)

weight_with_l2_loss = tf.add_n(tf.get_collection("losses"))
loss = tf.reduce_mean(cross_entrpy) + weight_with_l2_loss
...
def train():
    ...
    with tf.Session() as sess:
        ...
        for step in range(max_step):
            ...
            sess.run(train_step, feed_dict={x:x_batch, y_:y_batch, keep_prob:0.5, is_training:True})
            ...
            if step%250 == 0:
                ...
                for x_batch,y_batch in zip(x_train_batches,y_train_batches):
                    predicition,train_loss = sess.run([result,loss], feed_dict={x: x_batch, y_: y_batch, keep_prob:1, is_training:False})
                ...
                for x_batch,y_batch in zip(x_test_batches,y_test_batches):
                    predicition, test_loss = sess.run([result, loss], feed_dict={x: x_batch, y_: y_batch, keep_prob:1, is_training:False})
                    predicition = np.argmax(predicition, axis=1)
                    predicition = (predicition == y_batch.astype("int32"))
                    # print(predicition)
                    true_count += np.sum(predicition.astype("int32"))
        ...
        # saver.save(sess, "model/vgg16/vgg16.ckpt")
...
if __name__ == '__main__':
    ...
    #
    # writer = tf.summary.FileWriter("log/vgg16", tf.get_default_graph())
    # writer.close()
